CollectionHooks.defineAdvice("update",
  function (userId, _super, instance, aspects, getTransform, args, suppressAspects) {
      ...
      console.log("ARGS ARE UPDATE ", args);
      ...
      if (!suppressAspects) {
          try {
              if (aspects.before || aspects.after) {
                  ...
                  docIds = _.map(docs, function (doc) {
                      return doc._id;
                  });
              }
              ...
              if (aspects.after) {
                  ...
                  if (_.some(aspects.after, function (o) {
              ...
              _.each(aspects.before, function (o) {
                  _.each(docs, function (doc) {
                      var r = o.aspect.call(_.extend({transform: getTransform(doc)}, ctx), userId, doc, fields, args[1],
                        args[2]);
                      if (r === false) {
                          abort = true;
                      }
      ...
      if (async) {
          ...
          // console.log("ARGS UPDATE HERE ", args);
